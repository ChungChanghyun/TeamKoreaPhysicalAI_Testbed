...
class LayoutManager:
    ...
    def _generate_state_polygons_and_collisions(self, agv_length=2630, agv_width=1380):
        ...
        # for move_id, move_state in self.Move_state.items():
        #
        #     # estimate heading angle for stop states
        #     if not move_state.center:
        #         #backward movement의 vehicle 방향
        #         if int(move_state.segment_id) > 0:
        #             x1, y1 = self.node_coords[move_state.start_id]
        #             x2, y2 = self.node_coords[move_state.end_id]
        #             heading = np.degrees(np.arctan2(y2 - y1, x2 - x1))
        #             ##메모 heading list 형태면 append로 변경 필요
        #             # if not hasattr(self.Stop_state[move_state.start_id], 'heading'):
        #             self.Stop_state[move_state.start_id].heading = heading
        #             self.Stop_state[move_state.end_id].heading = heading
        #             # if not hasattr(self.Stop_state[move_state.end_id],'heading')
        #         else:
        #             x1, y1 = self.node_coords[move_state.start_id]
        #             x2, y2 = self.node_coords[move_state.end_id]
        #             heading = np.degrees(np.arctan2(y2 - y1, x2 - x1))
        ...
        #             self.Stop_state[move_state.end_id].heading = -heading
        for move_id, move_state in self.Move_state.items():
            poly = generate_move_polygon(move_state, self, self.length, self.width)
            self.move_regions[move_id] = poly

        for stop_id, stop_state in self.Stop_state.items():
            self.stop_regions[stop_id] = stop_state_to_polygon(stop_state, self, self.length, self.width)

        # collision detection
        idx = index.Index()
        for sid, poly in self.stop_regions.items():
            idx.insert(hash(sid), poly.bounds, obj=sid)
        for mid, poly in self.move_regions.items():
            idx.insert(hash(mid), poly.bounds, obj=mid)

        self.collision_profile = {**{sid: [] for sid in self.stop_regions}, **{mid: [] for mid in self.move_regions}}
        for sid, spoly in self.stop_regions.items():
            for mid, mpoly in self.move_regions.items():
                if spoly.intersects(mpoly):
                    self.collision_profile[sid].append(mid)
                    self.collision_profile[mid].append(sid)

        for sid, spoly in self.stop_regions.items():
            for mid, mpoly in self.stop_regions.items():
                if spoly.intersects(mpoly):
                    self.collision_profile[sid].append(mid)
                    self.collision_profile[mid].append(sid)

        for sid, spoly in self.move_regions.items():
            for mid, mpoly in self.move_regions.items():
                if spoly.intersects(mpoly):
                    self.collision_profile[sid].append(mid)
                    self.collision_profile[mid].append(sid)
    ...
    def fill_affect_state(self):
        for stop in self.Stop_state:
            self.Stop_state[stop].affect_state = self.collision_profile[stop]

        for move in self.Move_state:
            self.Move_state[move].affect_state = self.collision_profile[move]
...
if __name__ == '__main__':
    ...
    # --- 입력 / 출력 경로 설정 ---
    layout_filename = "Maps/sample_2.json"
    output_pkl      = "sample2_1104.pkl"
