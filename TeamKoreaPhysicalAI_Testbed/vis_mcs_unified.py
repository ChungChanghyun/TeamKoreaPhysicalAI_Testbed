...
class CombinedSimulator:
    ...
    def _init_mcs(self):
        ...
        if oht_port_nodes:
            self.mcs.register_system(
                ...
                port_prod_rate=0.1,  # loads/min per port
            )
    ...
    def _init_agv_agents(self):
        """AGV를 포트 위치에 DONE 상태로 배치. 경로 없음 — MCS가 할당할 때 출발."""
    ...
    def _extend_with_exit(self, done_ids: set, mcs_goals: set,
        ...
        for a in self.agv_agents:
            ...
            # 이 AGV의 목적지가 mcs_goals와 겹치는지 확인
            agv_goal = self._agv_goals.get(a.id)
            ...
            if exit_port is None:
                print(f'  [EXIT] A{a.id}: no exit port from {agv_goal} ({reason})')
                continue
            if exit_port == agv_goal:
            ...
            # 탈출 경로 SIPP 계획 (DONE agent 위치만 간단 constraint)
            exit_constraints = []
            ...
            if result and result.paths.get(a.id):
                ...
                if len(exit_path) >= 2:
                    ...
                    print(f'  [EXIT] A{a.id}: {reason} extended {agv_goal} → {exit_port} '
                          f'(+{len(ext)} steps, exit_t={ext[-1][1]:.2f})')
                else:
                    print(f'  [EXIT] A{a.id}: exit path too short ({reason})')
            else:
                print(f'  [EXIT] A{a.id}: exit FAILED {agv_goal} → {exit_port} ({reason})')
    ...
    def _replan_done_agvs(self, sim_time: float):
        ...
        # ── DEBUG: replan trigger 시점 상태 출력 ──
        print(f'\n{"="*70}')
        print(f'[REPLAN TRIGGER] t={sim_time:.2f}s  pending={done_ids}')

        # OHT 마지막 위치
        print(f'  OHT final positions:')
        for a in self.oht_agents:
            seg = a.path[-1] if a.path else '?'
            print(f'    OHT {a.id}: last_seg={seg}  state={a.state}')
        ...
        for a in self.agv_agents:
            ...
            moving = 'ACTIVE' if a.path_idx < len(a.raw_path) else 'DONE'
            print(f'    A{a.id}: pos={nid}  goal={goal}  '
        ...
        # 0.5) ACTIVE pushed AGV가 MCS goal 목적지로 향하고 있으면,
        #      기존 경로 끝에 탈출 경로를 이어붙여서 inf 점유를 해소
        mcs_goals_pre = {self._agv_goals.get(aid) for aid in done_ids
                         if self._agv_goals.get(aid)}
        if mcs_goals_pre:
            self._extend_with_exit(done_ids, mcs_goals_pre, sim_time)
        ...
        for aid, cur in positions_to_plan.items():
            ...
            if mcs_goal and mcs_goal != cur and aid not in self._agv_pushed:
                ...
                used.add(mcs_goal)
                print(f'  [GOAL] A{aid}: MCS goal → {mcs_goal} (cur={cur})')
                continue
            # idle/pushed AGV — 빈 port를 임시 목적지로 할당 (비켜주기)
            if aid in idle_aids or aid in self._agv_pushed:
                ...
                for p in ports:
                    if p != cur and p not in used:
                        new_goals[aid] = p
                        used.add(p)
                        reason = 'PUSH(idle blocker)' if aid in idle_aids else 'PUSH(re-push)'
                        print(f'  [GOAL] A{aid}: {reason} {cur} → {p}')
                        break
                else:
                    new_goals[aid] = cur
                    print(f'  [GOAL] A{aid}: PUSH FAILED — no free port, stay at {cur}')
                continue
            # MCS 목적지 없음 → 랜덤 포트 할당
            for p in ports:
                if p != cur and p not in used:
                    new_goals[aid] = p
                    used.add(p)
                    print(f'  [GOAL] A{aid}: random → {p} (cur={cur}, no MCS goal)')
                    break
            else:
                for p in ports:
                    if p != cur:
        ...
        # ── DEBUG: planning 대상 요약 ──
        print(f'  [PLAN INPUT] positions={positions_to_plan}')
        print(f'  [PLAN INPUT] goals={new_goals}')
        print(f'  [PLAN INPUT] idle_aids={idle_aids}  pushed={self._agv_pushed}')
        print(f'  [PLAN INPUT] #constraints={len(all_constraints)}')
        ...
        # 5) 경로 끝에 dwell time 추가 (MCS 작업 AGV만, push 제외)
        #    arrival_idx = dwell 추가 전 마지막 step → 물리적 도착 시점
        dwell = self.mcs.dwell_time
        for aid, new_path in result.paths.items():
            if aid in idle_aids or aid in self._agv_pushed:
                continue  # push된 AGV는 dwell 불필요
            if new_path and len(new_path) >= 2:
                arrival_idx = len(new_path) - 1  # dwell 추가 전 마지막 인덱스
                last_sid, last_t = new_path[-1]
                new_path.append((last_sid, last_t + dwell))
                self._agv_arrival_idx[aid] = arrival_idx
    ...
    def update(self, dt_real: float):
        ...
        # ── MCS: DONE 감지 → MCS에 도착 통보 ──
        # OHT DONE → MCS VEHICLE_ARRIVED
        # DONE 상태 유지: DES 엔진이 node blocker로 인식해야 겹침 방지됨
        from mcs_unified import post_vehicle_arrived
        for a in self.oht_agents:
            if a.state == DONE and a.id not in self._oht_done_notified:
                b = self.mcs.bindings.get(a.id)
                if b and b.load is not None:
                    post_vehicle_arrived(self._mcs_heap, self._mcs_seq,
        ...
        for a in self.agv_agents:
            ...
            if arr_idx is not None and a.path_idx >= arr_idx:
                b = self.mcs.bindings.get(a.id)
                if b and b.load is not None:
                    nid = a.raw_path[arr_idx][0].split(',')[1] if a.raw_path else '?'
                    print(f'[AGV ARRIVED] A{a.id} at {nid} — load={b.load.load_id} '
                          f'phase={b.phase.value} → VEHICLE_ARRIVED  t={self.sim_time:.2f}'
                          f'  (path_idx={a.path_idx}, arrival_idx={arr_idx})')
                    post_vehicle_arrived(self._mcs_heap, self._mcs_seq,
                                        a.id, b.token, self.sim_time)
        ...
        for a in self.agv_agents:
            if a.state == AGV_DONE and a.id not in self._agv_done_notified:
                ...
                if a.id in self._agv_pushed:
                    print(f'[AGV DONE] A{a.id} at {nid} — PUSHED, clear goal')
                    self._agv_goals.pop(a.id, None)
                    self._agv_done_notified.add(a.id)
                    continue
                # arrival_idx가 설정된 AGV는 위에서 이미 통보됨 → 중복 방지
                if a.id in self._agv_arrived_notified:
                    print(f'[AGV DONE] A{a.id} at {nid} — dwell complete (already notified)')
                    self._agv_done_notified.add(a.id)
                    continue
                # arrival_idx 없는 AGV (초기 배치 등) → 기존 방식
                b = self.mcs.bindings.get(a.id)
                if b and b.load is not None:
                    print(f'[AGV DONE] A{a.id} at {nid} — load={b.load.load_id} '
                          f'phase={b.phase.value} → VEHICLE_ARRIVED  t={self.sim_time:.2f}')
                    post_vehicle_arrived(self._mcs_heap, self._mcs_seq,
                                        a.id, b.token, self.sim_time)
                else:
                    print(f'[AGV DONE] A{a.id} at {nid} — no load (idle)')
                self._agv_done_notified.add(a.id)
    ...
    def _draw_agv_agents(self, v_len, v_wid, ww, wh):
        ...
        for a in self.agv_agents:
            ...
            # MCS phase 기준 색상: IDLE=하늘, 작업중=초록
            b = self.mcs.bindings.get(a.id)
            mcs_idle = (b is None or b.phase == VehicleJobState.IDLE)
            if mcs_idle:
                base = (80, 180, 255)   # 하늘색
            else:
                base = (100, 220, 120)  # 초록색
    ...
    def _draw_sidebar(self, wh):
        ...
        # MCS stats
        line('── MCS ──', f=self.font_b, col=(60, 220, 220))
        mcs_stats = self.mcs.stats_summary(self.sim_time)
        line(f'  Wait:{mcs_stats["waiting"]} Active:{mcs_stats["active"]} '
             f'Done:{mcs_stats["completed"]}', f=self.font_s)
        line(f'  Thru:{mcs_stats["throughput"]:.1f}/m '
             f'Cycle:{mcs_stats["avg_cycle"]:.1f}s '
             f'Util:{mcs_stats["utilization"]:.0%}', f=self.font_s)
        y += 6
