"""
vis_mcs.py — MCS(Material Control System) 통합 시뮬레이터 (KaistTB map)

OHT + AGV + 3DS + Elevator 통합 주행 + DES 기반 작업 생성/할당/KPI 추적.
기존 vis_combined.py에 MCS 관제 시스템을 추가한 버전입니다.

MCS: mcs.py의 MCSEngine — 포아송 작업 생성, FIFO 할당, pickup→delivery 2단계 수송
...
from mcs import MCSEngine, VehicleJobState
...
SIDE_W       = 460          # 2-column sidebar (left: vehicles, right: MCS)
MAP_W        = WIN_W - SIDE_W
SIDE_COL_W   = 230         # 각 열 너비
MAP_PAD      = 40
FPS          = 60
MAX_AGENTS   = 10
...
class CombinedSimulator:
    def __init__(self, oht_map: OHTMap, amr_graph: PklMapGraph,
        ...
        pygame.display.set_caption('MCS Simulator — KaistTB (Job Dispatch + KPI)')
        ...
        # MCS — Material Control System (작업 생성/할당/KPI)
        self._init_mcs()

    # ── Graph distance (precomputed) ────────────────────────────────────

    @staticmethod
    def _dijkstra_from(adj, edge_len_fn, src):
        """src에서 모든 노드까지의 최단거리 dict 반환."""
        import heapq as _hq
        dist = {src: 0.0}
        heap = [(0.0, src)]
        while heap:
    ...
    @staticmethod
    def _build_dist_table(adj, edge_len_fn, all_nodes):
        """모든 노드 → 모든 노드 최단거리 테이블. {(src, dst): dist}"""
        table = {}
        for src in all_nodes:
            dists = CombinedSimulator._dijkstra_from(adj, edge_len_fn, src)
            for dst, d in dists.items():
                table[(src, dst)] = d
        return table

    def _precompute_distances(self):
        """시뮬 초기화 시 한 번만 호출 — 모든 노드 간 최단경로 테이블 구축."""
        # OHT
        def oht_seg_len(u, v):
            seg = self.oht_map.segments.get((u, v))
            return seg.length if seg else float('inf')
        self._oht_dist = self._build_dist_table(
            self.oht_map.adj, oht_seg_len, list(self.oht_map.nodes.keys()))

        # AGV
        def agv_edge_len(u, v):
            e = self.amr_graph.edges.get((u, v))
            return e.length if e else float('inf')
        self._agv_dist = self._build_dist_table(
            self.amr_graph.adj, agv_edge_len, list(self.amr_graph.nodes.keys()))

        # 3DS (층별)
        self._3ds_dist = {}
        for fid, fd in self.s3d_floor_data.items():
            graph = fd['graph']
            def edge_len(u, v, _g=graph):
                e = _g.edges.get((u, v))
                return e.length if e else float('inf')
            self._3ds_dist[fid] = self._build_dist_table(
                graph.adj, edge_len, list(graph.nodes.keys()))

    def _oht_distance(self, src: str, dst: str) -> float:
        return self._oht_dist.get((src, dst), float('inf'))

    def _agv_distance(self, src: str, dst: str) -> float:
        return self._agv_dist.get((src, dst), float('inf'))

    def _3ds_distance(self, fid: str, src: str, dst: str) -> float:
        return self._3ds_dist.get(fid, {}).get((src, dst), float('inf'))

    def _get_oht_node(self, vid: int) -> str | None:
        a = next((x for x in self.oht_agents if x.id == vid), None)
        return a.cur_node if a else None

    def _get_agv_node(self, vid: int) -> str | None:
        a = next((x for x in self.agv_agents if x.id == vid), None)
        if a is None or not a.raw_path:
            return None
        return a.raw_path[-1][0].split(',')[1] if a.state == AGV_DONE \
            else a.raw_path[a.path_idx][0].split(',')[1]

    def _get_3ds_node(self, fid: str, vid: int) -> str | None:
        fd = self.s3d_floor_data.get(fid)
        if fd is None:
            return None
        a = fd['env'].agents.get(vid)
        if a is None or not a.raw_path:
            return None
    ...
    def _init_mcs(self):
        """각 시스템별 MCSEngine(DES) 초기화."""
        self._precompute_distances()
        seed_base = 42

        # OHT
        oht_ports = list(self.oht_map.port_nodes)
        self.mcs_oht = MCSEngine(
            'OHT', oht_ports,
            on_dispatch=self._dispatch_oht,
            is_vehicle_free=self._is_oht_free,
            get_vehicle_node=self._get_oht_node,
            get_distance=self._oht_distance,
            port_prod_rate=0.5, dwell_time=3.0, seed=seed_base)
        for a in self.oht_agents:
            self.mcs_oht.register_vehicle(a.id)

        # AGV
        agv_ports = list(self.agv_planner._port_nodes)
        self.mcs_agv = MCSEngine(
            'AGV', agv_ports,
            on_dispatch=self._dispatch_agv,
            is_vehicle_free=self._is_agv_free,
            get_vehicle_node=self._get_agv_node,
            get_distance=self._agv_distance,
            port_prod_rate=0.5, dwell_time=3.0, seed=seed_base + 1)
        for a in self.agv_agents:
            self.mcs_agv.register_vehicle(a.id)

        # 3DS (층별)
        self.mcs_3ds: dict = {}
        for fid, fd in self.s3d_floor_data.items():
            ports_3ds = list(fd['planner']._port_nodes)
            self.mcs_3ds[fid] = MCSEngine(
                fid, ports_3ds,
                on_dispatch=lambda vid, goal, t, _fid=fid: self._dispatch_3ds(_fid, vid, goal, t),
                is_vehicle_free=lambda vid, _fid=fid: self._is_3ds_free(_fid, vid),
                get_vehicle_node=lambda vid, _fid=fid: self._get_3ds_node(_fid, vid),
                get_distance=lambda src, dst, _fid=fid: self._3ds_distance(_fid, src, dst),
                port_prod_rate=0.3, dwell_time=2.0,
                seed=seed_base + 10 + hash(fid) % 100)
            for a in fd['agents']:
                self.mcs_3ds[fid].register_vehicle(a.id)
    ...
    def update(self, dt_real: float):
        ...
        # ── MCS: DONE 차량 → 도착 알림 (이벤트 처리 전에) ──
        self._mcs_notify_arrivals(self.sim_time)

        # ── MCS: DES 이벤트 처리 (작업 생성, dwell 완료, 할당) ──
        self.mcs_oht.step(self.sim_time)
        self.mcs_agv.step(self.sim_time)
        for fid in self.mcs_3ds:
            self.mcs_3ds[fid].step(self.sim_time)
    ...
    def _mcs_notify_arrivals(self, t: float):
        """DONE 상태의 차량 → MCS에 도착 알림 또는 할당 시도 트리거."""
        # OHT
        oht_need_assign = False
        for a in self.oht_agents:
            if a.state != DONE:
                continue
            phase = self.mcs_oht.get_phase(a.id)
            if phase in (VehicleJobState.TO_PICKUP, VehicleJobState.TO_DELIVERY):
                self.mcs_oht.notify_arrived(a.id, t)
            elif phase == VehicleJobState.IDLE:
                oht_need_assign = True
        if oht_need_assign and self.mcs_oht.total_waiting > 0:
            self.mcs_oht.request_assign(t)

        # AGV
        agv_need_assign = False
        for a in self.agv_agents:
            if a.state != AGV_DONE or a.id in self._agv_pending_replan:
                continue
            phase = self.mcs_agv.get_phase(a.id)
            if phase in (VehicleJobState.TO_PICKUP, VehicleJobState.TO_DELIVERY):
                self.mcs_agv.notify_arrived(a.id, t)
            elif phase == VehicleJobState.IDLE:
                agv_need_assign = True
        if agv_need_assign and self.mcs_agv.total_waiting > 0:
            self.mcs_agv.request_assign(t)

        # 3DS
        for fid, fd in self.s3d_floor_data.items():
            mcs = self.mcs_3ds[fid]
            s3d_need_assign = False
            for a in fd['agents']:
                if a.state != AGV_DONE or a.id in fd['pending_replan']:
                    continue
                phase = mcs.get_phase(a.id)
                if phase in (VehicleJobState.TO_PICKUP, VehicleJobState.TO_DELIVERY):
                    mcs.notify_arrived(a.id, t)
                elif phase == VehicleJobState.IDLE:
                    s3d_need_assign = True
            if s3d_need_assign and mcs.total_waiting > 0:
                mcs.request_assign(t)

    # ── MCS: transport layer 유휴 상태 확인 ────────────────────────────

    def _is_oht_free(self, vid: int) -> bool:
        """OHT 차량이 transport layer에서 DONE 상태인지."""
        a = next((x for x in self.oht_agents if x.id == vid), None)
        return a is not None and a.state == DONE

    def _is_agv_free(self, vid: int) -> bool:
        """AGV가 transport layer에서 DONE이고 replan 대기가 아닌지."""
        a = next((x for x in self.agv_agents if x.id == vid), None)
        return (a is not None and a.state == AGV_DONE
                and vid not in self._agv_pending_replan)

    def _is_3ds_free(self, fid: str, vid: int) -> bool:
        """3DS 셔틀이 transport layer에서 DONE이고 replan 대기가 아닌지."""
        fd = self.s3d_floor_data.get(fid)
        if fd is None:
            return False
        a = fd['env'].agents.get(vid)
        return (a is not None and a.state == AGV_DONE
                and vid not in fd['pending_replan'])

    # ── MCS dispatch callbacks (MCSEngine이 호출) ────────────────────────

    def _dispatch_oht(self, vehicle_id: int, goal_node: str, t: float):
        """OHT 차량에 이동 명령."""
        a = next((x for x in self.oht_agents if x.id == vehicle_id), None)
        if a is None:
            return
        path = self.oht_map.bfs_path(a.cur_node, goal_node)
        if len(path) > 1:
            self.oht_env.reassign(a, path, t)
        else:
            # 이미 해당 노드에 있음 → 즉시 도착 알림
            self.mcs_oht.notify_arrived(vehicle_id, t)

    def _dispatch_agv(self, vehicle_id: int, goal_node: str, t: float):
        """AGV 차량에 이동 명령 (SIPP replan)."""
        self._agv_pending_replan.add(vehicle_id)
        self._mcs_replan_agv_to_goal(vehicle_id, goal_node, t)

    def _dispatch_3ds(self, fid: str, vehicle_id: int, goal_node: str,
                      t: float):
        """3DS 셔틀에 이동 명령 (SIPP replan)."""
        fd = self.s3d_floor_data.get(fid)
        if fd is None:
            return
        fd['pending_replan'].add(vehicle_id)
        self._mcs_replan_3ds_to_goal(fid, vehicle_id, goal_node, t)

    def _mcs_replan_agv_to_goal(self, aid: int, goal_node: str, t: float):
        """AGV 한 대를 특정 goal_node로 replan."""
        done_ids = set(self._agv_pending_replan)
        all_constraints = []
        for a in self.agv_agents:
            if a.id in done_ids:
            ...
            if idx >= len(a.raw_path):
                ...
                continue
            claimed_path = a.raw_path[idx:a.claim_idx]
            for sid, cbs_t in claimed_path:
                all_constraints.append({
                    'agent': a.id, 'loc': sid,
                    'timestep': (t, float('inf')),
                })
            unclaimed = a.raw_path[a.claim_idx:]
            if unclaimed:
                cs = self.agv_planner._build_constraints(unclaimed, a.id)
                all_constraints.extend(cs)

        agent = next((x for x in self.agv_agents if x.id == aid), None)
        if agent is None:
            self._agv_pending_replan.discard(aid)
            return
        last_sid = agent.raw_path[-1][0]
        cur_nid = last_sid.split(',')[1]

        result = self.agv_planner.plan(
            {aid: cur_nid}, {aid: goal_node},
            existing_constraints=all_constraints,
            start_times={aid: t},
        )
        ...
        if result and result.paths.get(aid):
            new_path = result.paths[aid]
            if len(new_path) >= 2:
                self._agv_goals[aid] = goal_node
                self.agv_env.extend_agents_batch({aid: new_path}, t)
        self._agv_pending_replan.discard(aid)

    def _mcs_replan_3ds_to_goal(self, fid: str, aid: int, goal_node: str,
                                t: float):
        """3DS 셔틀 한 대를 특정 goal_node로 replan."""
        fd = self.s3d_floor_data[fid]
        planner = fd['planner']
        tapg_env = fd['env']
        agents = fd['agents']
        done_ids = set(fd['pending_replan'])
        ...
        a_obj = tapg_env.agents.get(aid)
        if a_obj is None or not a_obj.raw_path:
            fd['pending_replan'].discard(aid)
            return
        cur_nid = a_obj.raw_path[-1][0].split(',')[1]
        ...
        if result and result.paths.get(aid):
            new_path = result.paths[aid]
            if len(new_path) >= 2:
                fd['goals'][aid] = goal_node
                tapg_env.extend_agents_batch({aid: new_path}, t)
        fd['pending_replan'].discard(aid)
    ...
    def _del_oht(self):
        ...
        self.mcs_oht.unregister_vehicle(a.id)
        self._n_oht = len(self.oht_agents)
    ...
    def _add_agv(self):
        ...
        # 새 AGV를 MCS에 등록
        for a in self.agv_agents:
            if a.id not in self.mcs_agv.bindings:
                self.mcs_agv.register_vehicle(a.id)
    ...
    def _del_agv(self):
        ...
        self.mcs_agv.unregister_vehicle(a.id)
    ...
    def _draw_sidebar(self, wh):
        ...
        def line(txt, col=COL_TEXT, f=None, x_off=sx):
            nonlocal y
            fnt = f or self.font_m
            self.screen.blit(fnt.render(txt, True, col), (x_off, y))
        ...
        for a in self.oht_agents:
            ...
            mcs_ph = self.mcs_oht.get_phase(a.id).value[:5]
            self.screen.blit(
                self.font_s.render(
                    f' H{a.id:02d} {a.state:<9s} {mcs_ph}',
                    True, state_col),
                (sx+12, y))
            y += 14
        y += 4
        ...
        for a in self.agv_agents:
            ...
            pygame.draw.rect(self.screen, a.color,
                             pygame.Rect(sx, y+2, 10, 10), border_radius=2)
            display_id = a.id - 100
            mcs_ph = self.mcs_agv.get_phase(a.id).value[:5]
            st_short = a.state[:4] if a.state else '?'
            self.screen.blit(
                self.font_s.render(
                    f' A{display_id:02d} {st_short:<5s} v={a.v/1000:.1f} {mcs_ph}',
                    True, state_col),
                (sx+12, y))
            y += 14
        y += 6
        ...
        for fid in S3D_FLOOR_IDS:
            ...
            for a in fd['agents']:
                ...
                st_short = a.state[:4] if a.state else '?'
                mcs_ph = self.mcs_3ds.get(fid, self.mcs_oht).get_phase(a.id).value[:5] \
                         if fid in self.mcs_3ds else ''
                self.screen.blit(
                    self.font_s.render(
                        f' S{display_id} {floor_short} {st_short:<5s} {mcs_ph}',
                        True, state_col),
                    (sx+12, y))
                y += 14
        y += 6
        ...
        # Keys + area legend (bottom of left column)
        line('── Keys ──', f=self.font_b)
        for hint in ['SPACE start/pause', 'R reset  S shuffle',
                     '+/- speed  O/L OHT', 'N/P AGV  Q quit']:
            line(hint, col=COL_DIM, f=self.font_s)

        # ── Right column: MCS KPI ────────────────────────────────────────
        rx = MAP_W + SIDE_COL_W + 10
        # divider line
        pygame.draw.line(self.screen, (50, 55, 65),
                         (MAP_W + SIDE_COL_W, 0),
                         (MAP_W + SIDE_COL_W, wh), 1)

        ry = self._info_y

        def rline(txt, col=COL_TEXT, f=None):
            nonlocal ry
            fnt = f or self.font_m
            self.screen.blit(fnt.render(txt, True, col), (rx, ry))
            ry += fnt.get_linesize() + 2

        COL_MCS      = (100, 220, 255)
        COL_MCS_HEAD = (140, 200, 255)
        COL_MCS_VAL  = (180, 220, 255)
        COL_MCS_DIM  = (100, 130, 160)

        rline('── MCS (관제) ──', f=self.font_b, col=COL_MCS)
        ry += 2

        for sys_name, disp in [('OHT', self.mcs_oht), ('AGV', self.mcs_agv)] + \
                [(fid, self.mcs_3ds[fid]) for fid in sorted(self.mcs_3ds)]:
            st = disp.stats_summary(self.sim_time)
            short = sys_name.replace('3DS_', '')
            rline(f'── {short} ──', f=self.font_s, col=COL_MCS_HEAD)
            rline(f'  Wait   : {st["waiting"]:>4d}',
                  f=self.font_s, col=COL_MCS_VAL)
            rline(f'  Active : {st["active"]:>4d}',
                  f=self.font_s, col=COL_MCS_VAL)
            rline(f'  Done   : {st["completed"]:>4d}',
                  f=self.font_s, col=COL_MCS_VAL)
            if st['completed'] > 0:
                rline(f'  TPut   : {st["throughput"]:>5.1f} /min',
                      f=self.font_s, col=COL_MCS_DIM)
                rline(f'  CycleT : {st["avg_cycle"]:>5.1f} s',
                      f=self.font_s, col=COL_MCS_DIM)
                rline(f'  WaitT  : {st["avg_wait"]:>5.1f} s',
                      f=self.font_s, col=COL_MCS_DIM)
                rline(f'  Util   : {st["utilization"]*100:>5.1f} %',
                      f=self.font_s, col=COL_MCS_DIM)
            ry += 4

        # ── Active jobs detail ──
        ry += 2
        rline('── Active Loads ──', f=self.font_b, col=COL_MCS)
        all_engines = [('OHT', self.mcs_oht), ('AGV', self.mcs_agv)] + \
                      [(fid, self.mcs_3ds[fid]) for fid in sorted(self.mcs_3ds)]
        job_count = 0
        for sys_name, eng in all_engines:
            short = sys_name.replace('3DS_', '')
            for vid, b in eng.bindings.items():
                if b.load is None:
                    continue
                ld = b.load
                phase_short = b.phase.value[:5]
                src_s = ld.src_port[-4:] if len(ld.src_port) > 4 else ld.src_port
                dst_s = ld.dst_port[-4:] if len(ld.dst_port) > 4 else ld.dst_port
                rline(f'  L{ld.load_id:03d} {short} v{vid} {phase_short} '
                      f'{src_s}->{dst_s}',
                      f=self.font_s, col=COL_MCS_VAL)
                job_count += 1
                if job_count >= 15:
                    break
            if job_count >= 15:
                rline(f'  ... (more)', f=self.font_s, col=COL_MCS_DIM)
                break

        if job_count == 0:
            rline('  (none)', f=self.font_s, col=COL_MCS_DIM)

        # ── Completed jobs (recent) ──
        ry += 6
        rline('── Recent Done ──', f=self.font_b, col=COL_MCS)
        recent = []
        for _, eng in all_engines:
            recent.extend(eng.kpi.completed_loads[-5:])
        recent.sort(key=lambda l: l.t_completed, reverse=True)
        for ld in recent[:8]:
            ct = ld.cycle_time()
            short = ld.system.replace('3DS_', '')
            src_s = ld.src_port[-4:] if len(ld.src_port) > 4 else ld.src_port
            dst_s = ld.dst_port[-4:] if len(ld.dst_port) > 4 else ld.dst_port
            rline(f'  L{ld.load_id:03d} {short} {src_s}->{dst_s} {ct:.1f}s',
                  f=self.font_s, col=COL_MCS_DIM)
        if not recent:
            rline('  (none)', f=self.font_s, col=COL_MCS_DIM)
