...
class GraphDESv3:
    ...
    def _leader_extra_dist(self, leader: Vehicle, t: float) -> float:
        """How much further will leader travel beyond current position?
        Based on confirmed trajectory until next event + braking after."""
        t_confirmed = getattr(leader, 'next_event_t', t)
        if t_confirmed <= t:
            return leader.braking_distance(leader.vel_at(t))
        # Distance leader will travel from now until next event
        ...
        # After event, leader could brake
    ...
    def _maybe_notify(self, t: float, v: Vehicle, old_acc: float, old_vel: float):
        """If vehicle's motion changed significantly, notify followers."""
        if self._in_notify:
            return  # prevent recursive cascade
    ...
    def _is_zcu_blocked(self, zone: ZCUZone, seg_key: Tuple[str, str],
        ...
        # Curve segments: only 1 vehicle allowed (including same segment)
        if is_curve:
        ...
        # Check other entries/exits of this zone
        for other_key in zone.all_segs():
            ...
            if is_curve:
                return True  # curve blocked by any
            elif other_is_curve:
                return True  # straight blocked by curve
        return False

    def _free_dist_with_zcu(self, v: Vehicle, t: float) -> float:
        """Walk the vehicle's path, stopping at ZCU node boundaries.

        X marker = "guaranteed unobstructed path up to here".
        - X stays at nearest ZCU node until vehicle reaches braking distance
        - At braking distance: check if open → extend to next ZCU
        - Blocked → stop here
        """
        ...
        while dist < 100000 and pi + 1 < len(v.path) - 1:
            ...
            if next_node in self.gmap.merge_nodes or next_node in self.gmap.diverge_nodes:
                if dist > brake_dist:
                    # Far: X stays at this ZCU, don't look further
                    return dist

                # At braking distance: check if blocked
                blocked = False
                seg_key_in = (v.path[pi], next_node)
                zone = self.gmap.seg_to_zone.get(seg_key_in)
                if zone and self._is_zcu_blocked(zone, seg_key_in, v):
                    blocked = True
                if not blocked and pi + 2 < len(v.path):
                    seg_key_out = (next_node, v.path[pi + 2])
                ...
                # Open + within braking dist: extend to next ZCU only
                # Walk remaining path to find just the next ZCU node
                dist2 = dist
                pi2 = pi
                while pi2 + 1 < len(v.path) - 1:
                    tn2 = v.path[pi2 + 2] if pi2 + 2 < len(v.path) else None
                    if tn2 is None:
                        break
                    seg2 = self.gmap.segments.get((v.path[pi2 + 1], tn2))
                    if seg2:
                        dist2 += seg2.length
                    else:
                        break
                    pi2 += 1
                    next2 = v.path[pi2 + 1] if pi2 + 1 < len(v.path) else None
                    if next2 and (next2 in self.gmap.merge_nodes or next2 in self.gmap.diverge_nodes):
                        return dist2  # X at next ZCU
                return dist2 if dist2 < 100000 else float('inf')
