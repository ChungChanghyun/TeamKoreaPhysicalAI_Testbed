...
class Vehicle:
    def __init__(self, vid: int, track: Track, start_pos: float, color=(200,200,200)):
        ...
        # Next event time (for leader prediction)
...
class CircularDESv3:
    ...
    def _leader_confirmed_pos(self, leader: Vehicle, t: float) -> float:
        """Leader's confirmed future position based on its next event.
        Leader's kinematics are fixed until next_event_t, so pos_at(next_event_t)
        is guaranteed. After that, assume leader could brake immediately."""
        t_confirmed = leader.next_event_t
        if t_confirmed <= t:
            # Leader's event is in the past or now — use current pos + braking
            return leader.pos_at(t) + leader.braking_distance(leader.vel_at(t))
        # Position leader will definitely reach (current trajectory to next event)
        confirmed_pos = leader.pos_at(t_confirmed)
        # After next event, leader could brake — add braking distance at that speed
        confirmed_vel = leader.vel_at(t_confirmed)
        return confirmed_pos + leader.braking_distance(confirmed_vel)
    ...
    def _plan(self, t: float, v: Vehicle):
        ...
        # ── Free distance: based on leader's confirmed trajectory ────────
        # Leader's position is guaranteed until its next event.
        # After that, assume worst case (immediate braking).
        leader_confirmed = self._leader_confirmed_pos(leader, t)
        free_from_confirmed = ((leader_confirmed - v.pos) % self.track.length) - v.h_min
        free_from_gap = gap_d - v.h_min
        # Use confirmed position but never exceed current gap safety
        free_dist = min(free_from_confirmed, free_from_gap)
        ...
        # Notify followers if my motion changed
    ...
    def _maybe_notify(self, t: float, v: Vehicle, old_acc: float, old_vel: float):
        ...
        # Significant change: acceleration sign changed, or speed changed a lot
