...
from graph_des_v3 import (
    GraphMap, GraphDESv3, Vehicle, random_safe_path,
...
N_VEHICLES = 200
LEADER_REASSIGN_INTERVAL = 2.0  # seconds sim time
...
def main():
    ...
    # ── Create DES engine ─────────────────────────────────────────────────
    des = GraphDESv3(gmap, stop_prob=0.02, stop_dur=(2.0, 5.0))
    ...
    while running:
        ...
        if not paused:
            ...
            if sim_time - last_leader_reassign > LEADER_REASSIGN_INTERVAL:
                des.reassign_leaders_periodic(sim_time)
                last_leader_reassign = sim_time
        ...
        for v in vehicles:
            if v.stop_dist is not None and v.stop_dist > 0:
                ...
                stop_off = v.seg_offset + v._dist_traveled(sim_time - v.t_ref) + v.stop_dist
                pidx = v.path_idx
